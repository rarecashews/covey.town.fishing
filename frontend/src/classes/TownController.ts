...
import FishingAreaController from './interactable/FishingAreaController';
import TradingAreaController from './interactable/TradingAreaController';
import PlayerController from './PlayerController';
// eslint-disable-next-line import/no-extraneous-dependencies
import { addDoc, collection, deleteDoc, getDocs, limit, query } from '@firebase/firestore';
import { db } from './firebase';
import FishingDisplayAreaController from './interactable/FishingDisplayAreaController';
...
export default class TownController extends (EventEmitter as new () => TypedEmitter<TownEvents>) {
  ...
  // adds a fish game item to the firsestore for the current town to be persistent
  public async addFishToDisplayOnDB(item: GameItem, username: string) {
    try {
      let townName;
      if (this.friendlyName) {
        townName = this._friendlyNameInternal;
        if (townName) {
          await this.removeDisplayFishFromDB();
          addDoc(collection(db, 'towns', townName, 'inventory', 'displayArea', 'fish'), {
            fish: item,
            fisherman: username,
          });
        }
      }
      console.log('DIsplay written');
    } catch (e) {
      console.error('Error adding fish: ', e);
    }
  }

  // removes the currently displayed fish from the inventory in firestore
  public async removeDisplayFishFromDB() {
    try {
      let townName;
      if (this.friendlyName) {
        townName = this._friendlyNameInternal;
        if (townName) {
          ...
          const querySnapshot = await getDocs(query(fishCollectionRef, limit(1)));
      ...
      console.error('Error deleting fish collection: ', e);
    }
  }

  // retrieves the saved fish in the town's persistent display storage in firestore
  public async getDisplayFishFromDB(): Promise<[CatchableFish | undefined, string]> {
    const catchableFishes: CatchableFish[] = [];
    let username = '';
    try {
      let townName;
      if (this.friendlyName) {
        ...
        if (townName) {
          const querySnapshot = await getDocs(
            collection(db, 'towns', townName, 'inventory', 'displayArea', 'fish'),
          );
          querySnapshot.forEach(res => {
            const fishData = res.data().fish;
            ...
            catchableFishes.push(catchableFish);
            username = res.data().fisherman;
          });
          console.log('fish got', catchableFishes);
        }
      }
    } catch (e) {
      console.error('Error getting fish: ', e);
    }
    return [catchableFishes[0], username];
  }

  public get townPublicName() {
  ...
  public get fishDisplayAreas() {
    const ret = this._interactableControllers.filter(
      eachInteractable => eachInteractable instanceof FishingDisplayAreaController,
    );
    return ret as FishingDisplayAreaController[];
  }
  ...
  public get fishingAreas() {
    const ret = this._interactableControllers.filter(
      eachInteractable => eachInteractable instanceof FishingAreaController,
    );
    return ret as FishingAreaController[];
  }
  ...
  public interact<T extends Interactable>(interactedObj: T) {
    // console.log('pressed space:', interactedObj.getType());
  ...
  public async sendInteractableCommand<CommandType extends InteractableCommand>(
    ...
    return new Promise((resolve, reject) => {
      ...
      const ackListener = (response: InteractableCommandResponse<CommandType>) => {
        // console.log('commandResponse recieved with: ', response);
      ...
      // console.log('emitted interacableCommand:', commandMessage);
    });
  }
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      this._socket.on('initialize', initialData => {
        ...
        initialData.interactables.forEach(eachInteractable => {
          if (isConversationArea(eachInteractable)) {
          ...
          } else if (isFishDisplayArea(eachInteractable)) {
            this._interactableControllers.push(
              new FishingDisplayAreaController(eachInteractable, this),
            );
            ...
            this._interactableControllers.push(
              new TradingAreaController(eachInteractable.id, eachInteractable, this),
            );
          } else if (isFishingArea(eachInteractable)) {
            this._interactableControllers.push(new FishingAreaController(eachInteractable));
          }
        });
...
export function useFishingAreaController<T>(interactableAreaID: string): T {
  const townController = useTownController();
  const interactableFishingController = townController.fishingAreas.find(
    eachArea => eachArea.id == interactableAreaID,
  );
  if (!interactableFishingController) {
    throw new Error('Requested fishing area ${interactableAreaID} does not exist');
  }
  return interactableFishingController as unknown as T;
}
