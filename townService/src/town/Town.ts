...
import FishDisplayArea from './FishDisplayArea';
...
export default class Town {
  ...
  async addPlayer(userName: string, socket: CoveyTownSocket): Promise<Player> {
    ...
    socket.on('interactableCommand', (command: InteractableCommand & InteractableCommandBase) => {
      ...
      if (interactable) {
        try {
          // console.log('handleCommand is given:', command);
          // console.log('handleCommand is given:', command);
          const payload = interactable.handleCommand(command, newPlayer);
          // console.log('payload(responce from handleCommand):', payload);
          // console.log('payload(responce from handleCommand):', payload);
          ...
          // console.log(
          //   'emitted commandResponse with the following:',
          //   {
          //     commandID: command.commandID,
          //     interactableID: command.interactableID,
          //     isOK: true,
          ...
          // console.log('error emitting commandresponse');
  ...
  public initializeFromMap(map: ITiledMap) {
    ...
    const fishingAreas = objectLayer.objects
      .filter(eachObject => eachObject.type === 'FishingArea')
      .map(eachFishingAreaObject =>
        FishingArea.fromMapObject(eachFishingAreaObject, this._broadcastEmitter),
      );
    const fishDisplayAreas = objectLayer.objects
      .filter(eachObject => eachObject.type === 'FishDisplayArea')
      .map(eachFishingAreaObject =>
        FishDisplayArea.fromMapObject(eachFishingAreaObject, this._broadcastEmitter),
      );
